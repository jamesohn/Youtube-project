const config = require('config'),
  ...
  crawlerPqueue = new Pqueue({ concurrency: 100 }),
...
module.exports = class CommentThreads extends collector {
  async collect(channel) {
    ...
    await this.searchComment(channel,listOption,crawler)
    await crawler.onIdle()
    return `${channel} locale done`
  }

  async searchComment(channel,listOption,crawler) {
        ...
        try {
          if (item !== undefined && item.snippet.topLevelComment.snippet !== undefined) {
            ...
            if(!ids.has(authorChannelId)) {
              ...
              crawlerPqueue.add(() => crawler.concurrencyCollect(authorChannelId, textDisplay))
                .then((result) => {
                  ...
                  let filen = './' + 'locale_Test/' + channel + '_locale.txt'
                  fs.writeFile(filen, `totalCommentCount: ${ids.size}, crawledCount: ${count}\n${JSON.stringify(final)}\ncpm: ${cpm} 원`, 'utf8', function(err){
                    return 'done'
                  });
                })
